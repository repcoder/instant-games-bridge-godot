...
var _banner_state = Bridge.BannerState.HIDDEN
var _interstitial_state = Bridge.InterstitialState.CLOSED
var _rewarded_state = Bridge.RewardedState.CLOSED
...
func _set_banner_state(state):
	_banner_state = state
	banner_state_changed.emit(_banner_state)

func _set_interstitial_state(state):
	_interstitial_state = state
	interstitial_state_changed.emit(_interstitial_state)

func _set_rewarded_state(state):
	_rewarded_state = state
	rewarded_state_changed.emit(_rewarded_state)
